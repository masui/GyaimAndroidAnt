...
public class LocalDict {
	...
	//Log.v("Gyaim","Dict read end");
	initLink(); // 辞書エントリ間のリンク設定
	//Log.v("Gyaim","initLink end");
    }
	...
	//Log.v("Gyaim","GenerateCand("+word+","+pat+","+level+")");
	wordStack[level] = word;
	patStack[level] = pat;
    
	int patlen = cslength[len];
	int d = (connection != 0 ? connectionLink[connection] : keyLink[keylink]);
	for(;d >= 0 && Search.ncands < Gyaim.MAXCANDS;d = (connection != 0 ? dict[d].connectionLink : dict[d].keyLink)){
		...
		if(matchlen == patlen && (!exactMode || exactMode && dict[d].pat.length() == matchlen)){ // 最後までマッチ
		    ...
		    //Log.v("Gyaim","ncands = " + Search.ncands + ", fib1 = " + fib1);
